...
# input(): 
###   Función de entrada que captura datos del usuario como tipo String (STR)[cite: 125, 145, 152].
# f-strings: 
### Formato (f"") que permite incrustar variables en cadenas de texto de forma dinámica[cite: 148, 154, 155].

# 2. MÉTODOS DE MANIPULACIÓN DE STRINGS (CADENAS)

# .strip(): 
### Elimina espacios en blanco al inicio y final de una cadena de texto[cite: 35, 155].
# .lower(): 
### Normaliza el texto convirtiéndolo totalmente a minúsculas[cite: 23, 35, 163].
# .upper(): 
### Transforma el texto a mayúsculas, usado aquí para resaltar el rol activo[cite: 162].
# .isalpha(): 
### Método booleano que valida si un carácter pertenece estrictamente al alfabeto[cite: 36].

# 3. FUNCIONES DE CONVERSIÓN Y MEDICIÓN

# float(): 
# Realiza 'Casting' para transformar un STR numérico en un número decimal[cite: 126, 150, 153].
# len(): 
### Función que contabiliza la cantidad total de caracteres en una cadena o elementos en una lista[cite: 64, 76, 108].

# 4. HERRAMIENTAS DE LÓGICA Y TIEMPO

# datetime.now(): 
### Consulta el reloj y calendario real del sistema operativo[cite: 40].
# .strftime(): 
...
# while: 
### Bucle iterativo basado en una condición; se usa para mantener el agente activo[cite: 4, 13, 17].
# for: 
### Bucle de iteración finita; ideal para recorrer cada letra de una palabra[cite: 7, 20, 34, 107].
# if-elif-else: 
### Estructura condicional que actúa como el selector de comandos del agente[cite: 6, 32, 127].